...
void ServerImpl::Join() {
  ...
  cv.wait(lk, [this]() { return _n_current.load() == 0; });
  assert(_thread.joinable());
  _thread.join();
  close(_server_socket);
}

void ServerImpl::_runner(int client_socket) {
  ...
  try {
    ...
    while ((readed_bytes = read(client_socket, client_buffer,
                                sizeof(client_buffer))) > 0) {
      ...
      while (readed_bytes > 0) {
        ...
        if (command_to_execute && arg_remains > 0) {
          _logger->debug("Fill argument: {} bytes of {}", readed_bytes,
                         arg_remains);
          std::size_t to_read =
              std::min(arg_remains, std::size_t(readed_bytes));
    ...
    _logger->error("SERVER_ERROR {}: {}", client_socket, ex.what());
  }
  close(client_socket);
  _n_current--;
  cv.notify_one();
...
void ServerImpl::OnRun() {
  ...
  while (running.load()) {
    ...
    {
      if (_n_current < _n_accepted) {
        _n_current++;
        std::thread _t(&ServerImpl::_runner, this, client_socket);
        _t.detach();
      } else {
        _logger->error("Sorry, max number of threads is: {}", _n_accepted);
      }
    }
  }
